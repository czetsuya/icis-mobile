	...
	public class SQLHelper
	{
		...
		public static void InsertVariates(ref Study study) 
		{
			for(int i = 0; i < study.GetVariates().Count; i++)
			{
				...
				String sql = String.Format("INSERT INTO variate (study_id, scale_id, variate_name, variate_property, variate_scale, variate_method, variate_datatype) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}')", study.ID, obj.SCALEID, obj.NAME, obj.PROPERTY, obj.SCALEID, obj.METHOD, obj.DATATYPE);
				String id = DataAccess.Instance().Insert(sql);
